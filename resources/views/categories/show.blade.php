...
@section('content')
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;">
            <div>
                <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.25rem;">
                    ...
                    <h2 style="font-size:1.25rem;font-weight:700;margin:0;color:var(--c-text);">{{ $category->name }}</h2>
                </div>
                @if($category->notes)
                    <p style="color:var(--c-muted);margin:.5rem 0 0 0;font-size:.875rem;">{{ $category->notes }}</p>
                @endif
            </div>
            <div style="display:flex;gap:.5rem;flex-shrink:0;">
                ...
                <form autocomplete="off" action="{{ route('categories.destroy', $category) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.4rem;">
        ...
        <a href="{{ route('operations.index', ['category_id' => $category->id]) }}" style="font-size:.8rem;color:var(--c-accent);text-decoration:none;">
                ...
                <div class="move-row" onclick="window.location.href='{{ route('operations.show', $operation->id) }}'">
                    ...
                    <div class="move-body">
                        ...
                        <div class="move-subtitle">
                            ...
                            @if($operation->place)
                                · <a href="{{ route('places.show', $operation->place) }}" style="color:var(--c-muted);" onclick="event.stopPropagation()">{{ $operation->place->name }}</a>
                            @endif
                        </div>
                    </div>
